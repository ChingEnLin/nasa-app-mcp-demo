...
async def test_api_endpoints():
    ...
    async with httpx.AsyncClient() as client:
        ...
        end_date = "2024-01-07"
        response = await client.get(f"{BASE_URL}/neo?start_date={start_date}&end_date={end_date}")
        if response.status_code == 200:
            data = response.json()
        ...
        if response.status_code == 200:
            data = response.json()
            active_rovers = [name for name, info in data['rovers'].items() if info['status'] == 'active']
